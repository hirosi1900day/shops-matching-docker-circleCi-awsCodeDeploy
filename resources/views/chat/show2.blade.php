...
<div id="chat">
    <ul class="messages">
        ...
        <section v-if="m.user_id == {{Auth::id()}}">
            <li class="right-side">
                <a class="chat_user_name text-decoration-none" href="{{route('users.show',['user'=>Auth::id()])}}">
                    ...
                    <span v-text="m.message" class="chat_text shadow"></span>
                </a>
            </li>
        </section>
        <section v-else>
            <li class="left-side">
                <a class="chat_user_name text-decoration-none" href="{{route('users.show',['user'=>$user->id])}}">
                    <div class="img-and-name">
  ...
  <button @click="getMessages" class="button chat_message_redirect">メッセージを更新</button>
</div>
